...
public sealed class McpToolsCommand : AsyncCommand<McpToolsCommand.Settings>
{
    private readonly IUserPreferencesStore _store;
    private readonly IConnectionService _connections;

    public McpToolsCommand(IUserPreferencesStore store, IConnectionService connections)
    {
        _store = store;
    ...
    protected override async Task<int> ExecuteAsync(CommandContext context, Settings settings, CancellationToken ct)
    {
        ...
        Core.Interfaces.IActiveConnection conn = null!;
        ...
        try
        {
            var tools = conn.Tools;
            if (tools.Count == 0)
            {
                AnsiConsole.MarkupLine("[dim]No tools found.[/]");
                return 0;
            }
